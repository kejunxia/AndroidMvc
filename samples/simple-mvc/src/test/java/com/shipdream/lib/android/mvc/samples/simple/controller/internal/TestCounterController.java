...
public class TestCounterController {
    ...
    private CounterBasicController counterController;

    protected EventBus eventBusC;
    protected EventBus eventBusV;
    protected ExecutorService executorService;

    private void prepareGraph() throws ProvideException, ProviderConflictException, Graph.IllegalRootComponentException {
        eventBusC = new EventBusImpl();
        eventBusV = new EventBusImpl();
        executorService = mock(ExecutorService.class);

        Mvc.graph().setRootComponent(
                (MvcComponent) new MvcComponent("Root").register(new Object(){
            @Provides
    ...
    public void setUp() throws Exception {
        ...
        counterController = new CounterBasicController();

        //inject dependencies into controller
        Mvc.graph().inject(counterController);

        //init controller
        counterController.onCreated();
    }
    ...
    public void increment_should_post_counter_update_event_with_incremented_value() {
        ...
        class Monitor {
            void onEvent(CounterBasicController.EventC2V.OnCounterUpdated event) {
            }
        }
